...
async def test_memory_store_resource_crud() -> None:
    ...
    res = await store.get_resource("work", "/work/event1.ics")
    assert res is None
    ...
    await store.save_resource("work", resource)
    ...
    deleted = await store.delete_resource("work", "/work/event1.ics")
    assert deleted is True

    assert await store.get_resource("work", "/work/event1.ics") is None
    assert await store.get_etags("work") == {}

    # Deleting non-existent resource returns False
    assert await store.delete_resource("work", "/work/event1.ics") is False
